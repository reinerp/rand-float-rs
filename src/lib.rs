//! Perfect uniform floating-point random number generation.
//!
//! This crate is a Rust port of the *round-down* variant of the “perfect
//! floating-point random number generator” by the
//! [fp-rand](https://github.com/specbranch/fp-rand/) project (C++ and Go
//! reference implementations, plus an accompanying paper).
//!
//! # What “perfect” means
//!
//! The classic technique for turning random bits into a uniform double —
//! take 53 bits and multiply by 2⁻⁵³ — produces only 2⁵³ equally spaced
//! values. It cannot return most representable floating-point numbers in
//! [0, 1): everything below 2⁻⁵³ except 0 is unreachable, and in every
//! binade below ½ only a fraction of the representable values can occur.
//!
//! The functions in this crate instead return a value distributed *exactly*
//! as if a real number had been drawn uniformly from (0, 1) and then rounded
//! **down** (toward −∞) to a representable floating-point value. Every float
//! in [0, 1) — including every subnormal and 0 itself — is returned with
//! probability equal to the measure of the interval of reals that rounds
//! down to it.
//!
//! # Algorithm
//!
...
//!
//! ```
//! // A tiny SplitMix64 as the entropy source.
...
//! let y = perfect_fp_rs::f32_down(&mut next);
//! assert!((0.0..1.0).contains(&y));
//! ```
//!
//! # References
//!
//! - <https://github.com/specbranch/fp-rand/> — reference implementations
//!   and the paper describing the algorithm and its proof.
...
/// the reals that round down to it. See the [crate documentation](crate)
/// for the algorithm.
///
...
/// let mut state = 1u64;
/// let mut src = move || {
///     // (xorshift64, for illustration only)
///     state ^= state << 13;
///     state ^= state >> 7;
///     state ^= state << 17;
///     state
/// };
/// let x = perfect_fp_rs::f64_down(&mut src);
/// let y = perfect_fp_rs::f64_down(&mut src);
/// assert!(x != y);
/// ```
#[inline]
...
mod tests {
    ...
    fn range_and_moments_f64() {
        ...
        for _ in 0..n {
            let x = f64_down(|| splitmix64(&mut state));
    ...
    fn range_and_moments_f32() {
        ...
        let mut sum = 0.0f64;
        for _ in 0..n {
            let x = f32_down(|| splitmix64(&mut state));
            assert!((0.0..1.0).contains(&x), "out of range: {x}");
            sum += x as f64;
        }
        let mean = sum / n as f64;
    ...
    fn low_binades_are_reachable_and_correctly_distributed() {
        ...
        let mut hits = 0u32;
        for _ in 0..n {
            if f64_down(|| splitmix64(&mut state)) < threshold {
                hits += 1;
            }
        }
